...
public readonly record struct ValidationFailure
{
    ...
    /// </summary>
    /// <param name="info">The <see cref="ValidationRuleFailureInfo"/> containing metadata and configuration for this specific failure.</param>
    /// <param name="message">A descriptive message explaining the reason for this validation failure.</param>
    private ValidationFailure(ValidationRuleFailureInfo info, string message, FailureSeverity? failureSeverity = null)
    {
        Info = info;
        Message = message;
        Severity = failureSeverity ?? info.Severity;
    }
    ...
    /// <summary>
    /// Gets the internal <see cref="ValidationRuleFailureInfo"/> object that provides detailed metadata
    /// and configuration for this specific validation failure.
    /// </summary>
    internal readonly ValidationRuleFailureInfo Info { get; }
    ...
    /// </summary>
    /// <param name="info">The <see cref="ValidationRuleFailureInfo"/> containing metadata and configuration for the failure.</param>
    ...
    internal static ValidationFailure ForPropertyComponent(ValidationRuleFailureInfo info, string message)
        => new(info, message);

    internal static ValidationFailure ForPreValidationRule(PreValidationFailureInfo info, string message)
        => new(info, message);

    internal static ValidationFailure ForMissingPropertyValue(ValidatorRunCtx runCtx)
    {
