...
/// <see cref="CustomValidatorRunCtx{T, TExternalResources}"/>.
/// </summary>
...
internal sealed class CustomValidatorCore<T, TExternalResources> :
    // Inherits from ValidatorCore, specializing it with ExpressValidatorRunCtx as the context.
    ValidatorCore<T, TExternalResources, CustomValidatorRunCtx<T, TExternalResources>>
{
    /// <summary>
    /// Initializes a new instance of the <see cref="CustomValidatorCore{T, TExternalResources}"/> class.
    ...
    /// <param name="components">A list of compiled <see cref="IValidatorComponent{T, TExternalResources, TContex}"/> instances
    /// that define the primary validation logic for this core.</param>
    public CustomValidatorCore(
        string validatorName,
        List<string>? snapShots,
        List<IValidatorComponent<T, TExternalResources, CustomValidatorRunCtx<T, TExternalResources>>> components)
        : base(validatorName, snapShots, components) // Pass all parameters to the base ValidatorCore constructor.
    {
