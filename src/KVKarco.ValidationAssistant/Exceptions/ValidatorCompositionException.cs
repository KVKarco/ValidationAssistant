...
public sealed class ValidatorCompositionException : ValidationAssistantException
{
    internal ValidatorCompositionException(string message) : base(message)
    {
    }

    internal ValidatorCompositionException(string message, Exception innerException) : base(message, innerException)
    {
    }

    internal ValidatorCompositionException()
    {
    }
        ...
        [CallerArgumentExpression(nameof(selector))] string? selectorName = null)
    {
        if (selector is null)
        {
            throw new ValidatorCompositionException(
                $"Selector '{selectorName}' cannot be null.");
        }

        Expression body = selector.Body;

        // Rule 1: Cannot select the root parameter itself directly, e.g. `x => x`
        if (body is ParameterExpression)
        {
            throw new ValidatorCompositionException(
                "Cannot select the main instance for validation directly. A property or field must be selected.");
        }
