...
public abstract class CustomValidator<T, TExternalResources>
{
    ...
    public ValidatorRunResult Validate(T value, CultureInfo? culture = null)
    {
        ...
        var contex = ValidatorRunCtx.ForNewRunAsMainValidator(_core, value, _resources, culture);
...
/// It simplifies the instantiation of <see cref="CustomValidator{T, TExternalResources}"/> by automatically
/// providing an <see cref="EmptyValidationResources"/> instance.
/// </summary>
/// <typeparam name="T">The type of the instance that this validator will validate.</typeparam>
public abstract class CustomValidator<T> : CustomValidator<T, EmptyValidationResources>
{
    /// <summary>
    /// Initializes a new instance of the <see cref="CustomValidator{T}"/> class.
    /// It automatically passes an instance of <see cref="EmptyValidationResources.Empty"/>
    /// to the base <see cref="CustomValidator{T, TExternalResources}"/> constructor.
    /// </summary>
    protected CustomValidator() : base(EmptyValidationResources.Empty)
    {

    }

    internal bool IsValid(ValidatorRunCtx parentCtx, T value)
    {
        var ctx = ValidatorRunCtx.ForNewRunAsChildValidator(_core, value, _resources, parentCtx);
        _core.InternalValidate(ctx);
        return ctx.Result.IsValid;
    }

    internal async ValueTask<bool> IsValidAsync(ValidatorRunCtx parentCtx, T value, CancellationToken ct = default)
    {
        var ctx = ValidatorRunCtx.ForNewRunAsChildValidator(_core, value, _resources, parentCtx);
